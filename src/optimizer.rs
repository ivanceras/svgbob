...
impl Optimizer{
    ...
    fn is_edible(&self, loc:&Loc) -> bool{
        self.can_loc_reduce(&loc.top(),loc)
        || self.can_loc_reduce(&loc.left(),loc)
    ...
    fn trace_elements(&self, element: &Element, loc:&Loc) -> Element{
        match self.reduce(element, &loc.right()){
            Some(reduced) => {
                self.trace_elements(&reduced, &loc.right())
            },
            None => {
                match self.reduce(element, &loc.bottom()){
                    Some(reduced) => {
                        self.trace_elements(&reduced, &loc.bottom())
                    }
                    None => {
                        match self.reduce(element, &loc.bottom_right()){
                            Some(reduced) => {
                                self.trace_elements(&reduced, &loc.bottom_right())
                            }
                            None => {
        ...
        match elm{
            ...
            Element::Arc(s, e, r, sw ) => {
                ...
                if match_last_loc{
                    ...
                    paths.push_str(&format!(" M {} {} A {} {} 0 0 {} {} {}",s.x, s.y, r, r, sweep, e.x, e.y));
                }
                last_loc = Some(e.clone());
            }
